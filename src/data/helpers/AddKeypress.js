...
const addKeypress = selectedSamples => {
  ...
  samplesWithKeypress = selectedSamples.map((sample, index) => {
    // First keyboard row
    if (index < rowLength) {
      return {...sample, keypress: keypresses.row1[index]}
    }
    // Second keyboard row
    if (index >= rowLength && index < rowLength * 2) {
      return {...sample, keypress: keypresses.row2[index - rowLength]}
    }
    // Third keyboard row
    if (index >= rowLength * 2 && index < rowLength * 3) {
      return {...sample, keypress: keypresses.row3[index - rowLength * 2]}
    }
  })
